...
const CW = 960, CH = 540, GROUND_Y = 420
const GRAVITY = 1300, JUMP_VEL = -600, MOVE_SPD = 230

const P = {
  skyTop: '#020101', skyBot: '#1c0805', glowA: 'rgba(180,55,0,0.28)', glowB: 'rgba(100,20,0,0.12)',
  sea: '#050d18', seaLine: '#0c1c2e', foam: '#142438', gnd: '#46300e', gndEdge: '#654618',
  sand: '#957018', col: '#181008', colHi: '#2e2010', charBody: '#0e0802', charGold: '#c8961c',
  charSkin: '#7a4e1e', charCape: '#8b0000', charHelm: '#b88818', sword: '#8888a0',
  fire0: '#ff6600', fire1: '#e02800', fire2: '#ffaa00', promptBg: 'rgba(6,4,2,0.88)',
  promptBorder: '#c8961c', promptDone: '#3a2810', promptText: '#c8961c', hint: '#5a4028',
  uiBg: 'rgba(0,0,0,0.6)', npcBody: '#1a1a1a', npcHair: '#2a1a0a', npcRobe: '#4a3020',
}

interface Player { x: number; y: number; vx: number; vy: number; facing: 1 | -1; onGround: boolean; walkPhase: number }
interface LevelObj {
  id: string; x: number; y: number; w: number; h: number
  kind: 'col' | 'fire' | 'body' | 'tent' | 'cave' | 'ship' | 'portal' | 'npc'
  label: string; eventId?: string; isShip?: boolean; npcId?: string; portalTo?: string
}

const LOCATIONS: Record<string, { name: string; objs: LevelObj[]; startX: number; width: number }> = {
  troy: {
    name: 'Берег Трои',
    startX: 160,
    width: 2900,
    objs: [
      ...
      { id: 'apollo', x: 920, y: GROUND_Y - 250, w: 58, h: 250, kind: 'col', label: 'Храм Аполлона', eventId: 'apollo_temple' },
      ...
      { id: 'ship', x: 2280, y: GROUND_Y - 155, w: 290, h: 155, kind: 'ship', label: 'На корабль', isShip: true },
      { id: 'portal-north', x: 2700, y: GROUND_Y - 100, w: 120, h: 100, kind: 'portal', label: 'На север (Фракия)', portalTo: 'thrace' },
    ],
  },
  thrace: {
    name: 'Берег Фракии',
    ...
    width: 2400,
    objs: [
      { id: 'cave1', x: 280, y: GROUND_Y - 80, w: 100, h: 80, kind: 'cave', label: 'Древняя пещера' },
      { id: 'npc-warrior', x: 520, y: GROUND_Y - 48, w: 60, h: 48, kind: 'npc', label: 'Фракийский воин', npcId: 'warrior_thrace' },
      { id: 'tent2', x: 750, y: GROUND_Y - 95, w: 130, h: 95, kind: 'tent', label: 'Торговый стан' },
      { id: 'fire3', x: 1050, y: GROUND_Y - 70, w: 28, h: 70, kind: 'fire', label: '' },
      { id: 'npc-priestess', x: 1300, y: GROUND_Y - 48, w: 60, h: 48, kind: 'npc', label: 'Жрица Деметры', npcId: 'priestess_thrace' },
      { id: 'col-temple', x: 1600, y: GROUND_Y - 280, w: 65, h: 280, kind: 'col', label: 'Святилище' },
      { id: 'portal-south', x: 2100, y: GROUND_Y - 100, w: 120, h: 100, kind: 'portal', label: 'Вернуться на юг', portalTo: 'troy' },
    ],
  },
}

const NPCS: Record<string, { name: string; role: string; greeting: string }> = {
  warrior_thrace: { name: 'Алексий', role: 'Фракийский воин', greeting: 'Незнакомец, откуда ты? Ахейец?' },
  priestess_thrace: { name: 'Артемиса', role: 'Жрица Деметры', greeting: 'Добро пожаловать в святилище. Дух твой нуждается в очищении.' },
}
...
function drawPortal(ctx: CanvasRenderingContext2D, o: LevelObj, cx: number, time: number) {
  ...
  ctx.fillStyle = `rgba(120, 60, 180, ${0.15 * pulse})`
  ctx.fill()
  ctx.strokeStyle = `rgba(180, 120, 220, ${0.7 + 0.3 * pulse})`
  ctx.lineWidth = 2.5
  ctx.stroke()
  ctx.restore()
}
...
function drawNPC(ctx: CanvasRenderingContext2D, o: LevelObj, cx: number, time: number) {
  const x = o.x - cx + o.w / 2, y = o.y + o.h - 44
  ...
  // legs
  ctx.fillStyle = P.npcBody
  ctx.fillRect(x - 6, y - 22, 5, 22)
  ctx.fillRect(x + 1, y - 22, 5, 22)

  // body
  rr(ctx, x - 10, y - 42 + bob, 20, 20, 3)
  ctx.fillStyle = P.npcRobe; ctx.fill()
  ctx.strokeStyle = '#000'; ctx.lineWidth = 2; ctx.stroke()

  // head
  ctx.beginPath(); ctx.arc(x, y - 50 + bob, 8, 0, Math.PI * 2)
  ctx.fillStyle = P.charSkin; ctx.fill()
  ctx.strokeStyle = '#000'; ctx.lineWidth = 2; ctx.stroke()

  // hair
  ctx.beginPath(); ctx.arc(x, y - 55 + bob, 9, 0, Math.PI, true)
  ctx.fillStyle = P.npcHair; ctx.fill()
  ctx.strokeStyle = '#000'; ctx.lineWidth = 1.5; ctx.stroke()
}
...
export function GameCanvas({ onExit, onTriggerEvent }: Props) {
  ...
  useEffect(() => {
    ...
    let currentLocKey = 'troy'
    let player: Player = { x: LOCATIONS.troy.startX, y: GROUND_Y, vx: 0, vy: 0, facing: 1, onGround: true, walkPhase: 0 }
    const objs = (LOCATIONS.troy.objs as (LevelObj & { done: boolean })[]).map(o => ({ ...o, done: false }))
    ...
    function switchLocation(locKey: string) {
      currentLocKey = locKey
      const newLoc = LOCATIONS[locKey]
      player.x = newLoc.startX
      player.vx = 0
      player.vy = 0
      player.onGround = true
      objs.length = 0
      objs.push(...(newLoc.objs as (LevelObj & { done: boolean })[]).map(o => ({ ...o, done: false })))
      for (const o of objs) {
        if (o.eventId && completedRef.current.includes(o.eventId)) o.done = true
      }
      cam = 0
    ...
    function onKeyDown(e: KeyboardEvent) {
      keys.add(e.code)
      if (['Space', 'ArrowUp', 'ArrowLeft', 'ArrowRight', 'ArrowDown', 'KeyA', 'KeyD', 'KeyW', 'KeyE'].includes(e.code)) e.preventDefault()
      ...
      if (e.code === 'KeyE' || e.code === 'Enter') {
        ...
        if (near.portalTo) { switchLocation(near.portalTo); return }
        if (near.kind === 'npc') {
          const npc = NPCS[near.npcId!]
          if (npc) alert(`${npc.name}, ${npc.role}:\n\n"${npc.greeting}"`)
          return
        }
    ...
    function loop(ms: number) {
      ...
      for (const o of objs) {
        ...
        if (o.kind === 'body') drawNPC(ctx, o, cam, time)
        ...
        if (o.kind === 'npc') drawNPC(ctx, o, cam, time)
      }
      ...
      // location name
      ctx.fillStyle = P.uiBg; rr(ctx, 10, 10, 280, 26, 4); ctx.fill()
      ctx.fillStyle = P.promptText
      ctx.font = 'bold 14px Cinzel, serif'
      ctx.fillText(LOCATIONS[currentLocKey].name, 18, 32)
      ctx.textAlign = 'left'
